...
export default async function UniversityDetailPage(
  ...
  return (
    <div className="space-y-10 px-4 py-8 sm:px-6 lg:px-8">
      {/* ----- HERO HEADER ----- */}
      <header className="relative overflow-hidden rounded-3xl px-6 py-12 sm:px-10 sm:py-16">
        <div className="relative z-10">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-secondary/80 px-4 py-1.5 text-sm font-medium text-secondary-foreground backdrop-blur-sm">
      ...
      <div className="grid gap-8 lg:grid-cols-3">
              ...
              <div className="flex flex-wrap gap-2">
                {university.majors.map((m) => (
                  <Badge
                    key={m.majorId}
                    variant="secondary"
                    className="rounded-full border-0 bg-secondary px-4 py-1.5 font-medium text-secondary-foreground transition hover:bg-secondary/80"
                  >
                    {m.major.name}
                  </Badge>
                ))}
                      ...
                      {university.subjectReqs.map((req) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-2.5">
                            <span className="inline-flex min-w-9 items-center justify-center rounded-full bg-secondary px-3 py-0.5 text-xs font-bold text-secondary-foreground">
                              {req.minMark}
                            </span>
                          </td>
                        </tr>
            ...
            <Card className="overflow-hidden border-0 bg-card/80 backdrop-blur-sm transition">
              ...
              <CardContent className="space-y-3 pt-4">
                {university.scholarships.map((s) => (
                  <div
                    key={s.id}
                    className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border bg-card/60 px-5 py-3 transition hover:bg-accent/60"
                  >
            ...
            <CardContent className="space-y-4 pt-4">
              {university.reviews.length === 0 ? (
                ...
                university.reviews.map((review) => (
                  <div
                    key={review.id}
                    className="rounded-xl border border-border bg-card/60 px-4 py-3 transition hover:bg-card"
                  >
        ...
        <div className="space-y-8 lg:sticky lg:top-24 lg:self-start">
          {/* Map */}
          {hasCoordinates && (
            <div className="overflow-hidden rounded-2xl border border-border bg-card/80 p-2 backdrop-blur-sm transition">
          ...
          {/* Review Form */}
          <div className="rounded-2xl border border-border bg-card/80 p-6 backdrop-blur-sm transition">
            <ReviewForm
